...
```text
- 地理范围：70-111°E, 25-50°N（西部 7 省：新疆、西藏、青海、四川、甘肃、宁夏、重庆）。
- 使用固定区域底图/边界图，底图浅色，不能抢占降水色斑主视觉。
- 省界/市界：浅灰线（7 省边界均需绘制），选中省界可加深。
- 省会标注：乌鲁木齐、拉萨、西宁、兰州、银川、成都、重庆（共 7 个），深灰色。
- 地图控件固定在右上或左上，不遮挡选区。
```

> **注意**：当前效果图 F01-F06 仅展示了陕西省附近区域，与实际业务范围（西部 7 省全域）不一致。后续重新出图时必须以 70-111°E, 25-50°N 全域为准，展示 7 省省界和 7 个省会城市标注。
...
8. 地图底图必须覆盖西部 7 省全域（70-111°E, 25-50°N），省界和省会标注以参考绘图代码 `WESTERN_PROVINCES` 为准。
