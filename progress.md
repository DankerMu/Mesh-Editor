...
**阶段：M0 Issue #6 DevOps/CI/测试整合完成，M0 工程骨架收尾**
...
- [x] M0 Issue #5 PR #10 Round 1 cross-review 修复完成（session 恢复 user 元数据、HTTP 403/5xx 路由、登录页固定错误提示）
- [x] M0 Issue #6 DevOps/CI/测试整合完成（Dockerfile、docker-compose、nginx、GitHub Actions CI、smoke test）

### 进行中

- [ ] M0 收尾：PR 审查、CI 远端结果确认
...
| 日期 | 变更 |
|---|---|
| 2026-05-16 | **M0 Issue #6 完成**：新增 backend/frontend Dockerfile、frontend nginx、docker-compose 命名卷与 healthcheck、GitHub Actions CI 六 job；补齐 ruff/mypy dev 依赖与前端测试 stub；本地 lint/type/test/migration/docker compose smoke 通过 |
| 2026-05-16 | **PR #10 Round 1 修复**：完成前端 authStore user 持久化恢复、HTTP interceptor 403/5xx/USER_DISABLED 路由处理、登录页固定错误提示与回归测试；`npm run build`、`npx vitest run --reporter=verbose` 通过 |
