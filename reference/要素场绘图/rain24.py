...
def plot_rain(year,month,day,hour,validtime1,validtime2,lenth,level,region):
    ...
    output_dir = '../images/'+fcst_year+fcst_month+fcst_day+fcst_hour+'/'+'rain'+str(lenth)+'/'+region+'/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_name = 'rain'+str(lenth)+'_'+region+'_'+fcst_year+fcst_month+fcst_day+fcst_hour+'_'+validtime1+'_'+str(level)+'hPa.png'
    ...
    if os.path.exists(input_file00) and os.path.exists(input_file01) and not os.path.exists(output_file):
        ...
        WESTERN_PROVINCES = ['新疆维吾尔自治区', '西藏自治区', '青海省',
                    '四川省', '甘肃省', '宁夏回族自治区', '重庆市']
        COUNTRY_BOUNDARY = "./nation.json"
        PROVINCE_BOUNDARY = "./province.json"
