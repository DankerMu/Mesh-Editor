...
def plot_circulation(year,month,day,hour,valid_time,level,region):
    ...
    input_file_prmsl = '/cloud/share-files/SC/mdfs/ECMWF_HR/PRMSL/'+'/'+fcst_year+fcst_month+fcst_day+fcst_hour+'/'+fcst_year[2:]+fcst_month+fcst_day+fcst_hour+'.'+valid_time
    input_file_wind = '/cloud/share-files/SC/mdfs/ECMWF_HR/WIND_10M/'+'/'+fcst_year+fcst_month+fcst_day+fcst_hour+'/'+fcst_year[2:]+fcst_month+fcst_day+fcst_hour+'.'+valid_time
    print(input_file_prmsl)
    print(input_file_wind)
    ...
    print(output_file)
    if os.path.exists(input_file_prmsl) and os.path.exists(input_file_wind) and not os.path.exists(output_file):
        data_prmsl = meb.read_griddata_from_gds_file(input_file_prmsl)
        data_wind = meb.read_gridwind_from_gds_file(input_file_wind)
        #0.取时间
        start_time, forecast_time = process_time_info(data_prmsl,valid_time)
        #1.取经纬度
        lon = data_prmsl['lon'].data
        lat = data_prmsl['lat'].data
        lons,lats = np.meshgrid(lon,lat)
        #2.取数据
        data_prmsl = data_prmsl.isel(level=0,time=0,dtime=0,member=0).values
        ...
        # 5. 绘制海平面气压填色图
        clevs_mslp = np.arange(960, 1041, 4)  # 气压范围960-1040hPa，间隔4hPa
        mslp_plot = ax.contourf(
            lons,lats,data_prmsl,  # 
            levels=clevs_mslp,
            cmap='coolwarm_r',
        ...
        cbar.set_label('海平面气压 (hPa)',fontproperties=font_prop, fontsize=12)
        # 6. 绘制等压线
        ...
        ax.clabel(mslp_contour, inline=True, fontsize=10, fmt='%d')
        # 7. 绘制风场（风向杆）
        skip = {'All':20,
              'NW':12,
              'SW':12,
        ...
        ax.barbs(
            ...
            barbcolor='darkgreen',               # 风杆颜色
            flagcolor='darkgreen',               # 三角旗颜色
            linewidth=0.8,                   # 线宽
