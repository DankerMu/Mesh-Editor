...
def plot_cape(year,month,day,hour,valid_time,level,region):
    extent = {'All':[70,115,25,50],
              'NW':[70,105,30,50],
              'SW':[75,100,25,40],
              'East':[90,115,25,40]
        }
    input_file_cape = '/cloud/share-files/SC/mdfs/ECMWF_HR/CAPE/'+'/'+fcst_year+fcst_month+fcst_day+fcst_hour+'/'+fcst_year[2:]+fcst_month+fcst_day+fcst_hour+'.'+valid_time
    print(input_file_cape)
    output_dir = '/gaoli/huishang/static/images/'+fcst_year+fcst_month+fcst_day+fcst_hour+'/'+'cape'+'/'+region+'/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_name = 'cape'+'_'+region+'_'+str(level)+'hPa_'+fcst_year+fcst_month+fcst_day+fcst_hour+'_'+valid_time+'.png'
    output_file = output_dir + output_name
    print(output_file)
    if os.path.exists(input_file_cape) and not os.path.exists(output_file):
        data_cape = meb.read_griddata_from_gds_file(input_file_cape)
        #0.取时间
        start_time, forecast_time = process_time_info(data_cape,valid_time)
        #1.取经纬度
        lon = data_cape['lon'].data
        lat = data_cape['lat'].data
        lons,lats = np.meshgrid(lon,lat)
        #2.取数据
        data_cape = data_cape.isel(level=0,time=0,dtime=0,member=0).values
        ...
        # 5. 绘制对流有效位能填色图
        clevs_cape = np.arange(0, 2001, 400)  # cape0-2000，间隔400
        colors_cape = ['#2b83ba', '#abdda4', '#ffffbf', '#fdae61', '#d7191c','#800026']
        ...
        cbar.set_label('对流有效位能(J)',fontproperties=font_prop, fontsize=12)
        # 6.绘制边界
        gdf_province.plot(
            ax=ax,
            facecolor="none",
        ...
        ax.set_title(text[region]+'对流有效位能',fontproperties=font_prop,fontsize=20)
...
for level in levels:
    for h in range(0,241,6):                        
        for region in regions:
            hh = str(h).zfill(3)
            plot_cape(fcst_year,fcst_month,fcst_day,fcst_hour,hh,level,region)
t1 = datetime.now()
print((t1-t0)/60)