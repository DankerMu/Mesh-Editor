...
LOCAL_GDS_DATA_ROOT = "/cloud/share-files/SC/mdfs/ECMWF_HR"  # TODO: 修改为您的本地数据根目录
...
def load_all_stations_data(init_time_dt):
    ...
    """
    print(f"开始加载所有站点数据，起报时间: {init_time_dt.strftime('%Y-%m-%d %HBJT')}")
...
def create_profile_plot_from_preloaded(station_name, lon, lat, init_time_dt, station_data):
   ...
   # new_colors = orig_cmap(np.linspace(0, 1, orig_cmap.N))
    ...
    cbar.set_label('相对湿度 (%)', fontsize=PLOT_CONFIG['tick_label_fontsize'],fontproperties=font_prop)
    cbar.ax.tick_params(labelsize=PLOT_CONFIG['tick_label_fontsize'] - 1)
    cbar.set_ticks(np.arange(50, 110, 10))
    cbar.set_ticklabels(['{:.0f}%'.format(x) for x in np.arange(50, 110, 10)])
    ...
    ax_upper.set_ylabel('气压 (hPa)', fontsize=PLOT_CONFIG['ylabel_fontsize'],fontproperties=font_prop)
    ...
    ax_lower.set_ylabel('2米温度 (°C)',
                        color=PLOT_CONFIG['t2m_color'],
                        fontsize=PLOT_CONFIG['ylabel_fontsize'],fontproperties=font_prop)
    ...
    # 设置标题
    title = f"EC细网格预报 剖面分析 [{station_name} {lon:.2f}E, {lat:.2f}N]\n起报时间: {init_time_dt.strftime('%Y-%m-%d %HBJT')}"
    fig.suptitle(title, fontsize=PLOT_CONFIG['title_fontsize'], fontproperties=font_prop, y=0.98)
    ...
    output_dir = os.path.join("/gaoli/huishang/static/images", init_time_str, "profile", province)
...
if __name__ == "__main__":
    ...
    print(f"自动选择起报时间: {target_time.strftime('%Y-%m-%d %HBJT')}")
