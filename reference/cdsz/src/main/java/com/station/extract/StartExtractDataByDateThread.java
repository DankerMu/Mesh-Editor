...
public class StartExtractDataByDateThread implements Runnable{
...
//	            dataType     date
    ...
    private int count;
	public StartExtractDataByDateThread(Map<String, double[]> stationInfo, Map<String, Map<String, List<String>>> filePathMapList, Date startDate, Date endDate, int count) {
...
//		     date       filepath
		...
		for(String date : map.keySet())
		{
			if(ii != 0 && ii % 10 == 0)
			{
				executorService.execute(new ExtractDataByDateThread(pathList, stationInfo, latch));
				pathList = new ArrayList<>();
				pathList.addAll(map.get(date));
			}
			else
			{
				pathList.addAll(map.get(date));
			...
			ii++;
//			executorService.execute(new ExtractDataByDateThread(map.get(date), stationInfo, latch));
//			System.out.println(date + " 抽取任务已开始...");
		}
		executorService.execute(new ExtractDataByDateThread(pathList, stationInfo, latch));
...
//			for(String date : filePathMapList.get(dataType).keySet())
//			{
//				executorService.execute(new ExtractDataByDateThread(dataType, filePathMapList.get(dataType).get(date), stationInfo, latch));
