 ...
 * @description TODO
 */
public class ProvinceBoundaryDrawerBack {
...
//		String filePathPng = "E:/fl/datas/images/deep/tp_999_deeplearning_2025112108_051_048.png";
		String outPath = "test.png";
		String basePath = "E:/fl/datas/images/";
		ProvinceBoundaryDrawerBack drawer = new ProvinceBoundaryDrawerBack();
...
//        String tuli = basePath + "tuli.png";
        try {
            ...
            Font font4 = font.deriveFont(Font.BOLD, 40);
            Font font6 = font.deriveFont(Font.BOLD, 50);
            ...
            for(String city : cityMap.keySet())
            {
            	...
            	x = transLon(lonlat[0], width);
            	y = transLat(lonlat[1], height);
//            	g2d.drawOval((int)x, (int)y, 100, 100);
            	...
            	if("加勒万".equals(city))
            	{
            		g2d.fillOval((int)x + 5, (int)y - 5, redis * 2, redis * 2);
            	}
            	else
            	{
            		g2d.fillOval((int)x - redis, (int)y - redis, redis * 2, redis * 2);
            	}
            	float lon = (float)x - (city.length() * 20);
            	float lat = (float)y + 60;
            	if("酒泉,武威".contains(city))//左
            	{
            		lon = (float)x - (city.length() * 50);
            		lat = (float)y + 15;
            	}
            ...
            int addWidth = 0;
            int addHeight = 0;
            BufferedImage image = new BufferedImage(width + addWidth * 2, height + addHeight * 2, BufferedImage.TYPE_INT_RGB);
            Graphics2D graphics = image.createGraphics();
            graphics.setColor(Color.WHITE);
            graphics.fillRect(0, 0, width + addWidth * 2, height + addHeight * 2);
            graphics.drawImage(image1, 0 + addWidth - 100, 80 + addHeight, null);
...
//              drawProvinceName(g2d, province, path);
            ...
            graphics2.drawLine(0, image2.getHeight() - 1, image2.getWidth(), image2.getHeight() - 1);
//            graphics2.setFont(new Font("宋体", Font.PLAIN, 40));
            Font font1 = font.deriveFont(Font.PLAIN, 100);
...
//            String title = "战区未来24小时天气趋势预报";
            graphics2.drawString(title, image2.getWidth() / 3, addHeight + 80);
            
            Font font3 = font.deriveFont(Font.PLAIN, 60);
...
//            String subTitle = month + "月" + day + "日20时～" + nextDay + "日20时";
            ...
            graphics2.drawString(subTitle, (int) (image2.getWidth() / 2.38), 90 + 130);
            
//            System.out.println("图例图片路径:" + tuli);
//            BufferedImage tuLiImage = ImageIO.read(new File(tuli));
//            graphics2.drawImage(tuLiImage, image2.getWidth() - 200, 210 , null);

            
            Font font5 = font.deriveFont(Font.BOLD, 70);
            graphics2.setFont(font5);
            graphics2.drawString("图 例", image2.getWidth() - 310, 180);
            
            int i = 0;
//            graphics2.setFont(new Font("黑体", Font.PLAIN, 15));
            ...
            for(int[] color : rainColorList)
            {
            	...
            	graphics2.fillRect(width + addWidth - tuLiXBaseTu, tuLiYBase + i * (tuLiHeight + 10), tuLiWidth, tuLiHeight);
            	graphics2.setColor(Color.BLACK);
            	graphics2.drawString(rainColorNameValues[i], width + addWidth - tuLiXBaseZi, tuLiYBase + i * (tuLiHeight + 10) + 45);
                i++;
            }
...
//            i = 0;
            for(int[] color : snowColorList)
            {
            	...
            	graphics2.fillRect(width + addWidth - tuLiXBaseTu, tuLiYBase + i * (tuLiHeight + 10), tuLiWidth, tuLiHeight);
            	graphics2.setColor(Color.BLACK);
            	graphics2.drawString(snowColorNameValues[i - rainColorNameValues.length], width + addWidth - tuLiXBaseZi, tuLiYBase + i * (tuLiHeight + 10) + 45);
                i++;
            }
...
//            File fontFile = new File("E:/font/Weather_std_htht.ttf");
            ...
            try {
                customFont = Font.createFont(Font.TRUETYPE_FONT, fontFile).deriveFont(Font.PLAIN, 100);
         ...
         // 使用自定义字体绘制符号
            if (customFont != null) {
                ...
                for(int j = 0, count = wwColorNameValues.length; j < count; j++)
                {
                	...
                	graphics2.drawString(ws.weather(17), width + addWidth - tuLiXBaseTu + 40, tuLiYBase + i * (tuLiHeight + 10) + 65); // 位置(x, y)
                	graphics2.setFont(font2);
                	graphics2.setColor(Color.BLACK);
                	graphics2.drawString(wwColorNameValues[j], width + addWidth - tuLiXBaseZi, tuLiYBase + i * (tuLiHeight + 10) + 45);
                    i++;
                }
            }
...
//        Point firstPoint = latLonToScreen(points.get(0));
        ...
        double lat = transLat(points.get(0).latitude, height) + 80;
        path.moveTo(lon, lat);
...
//            Point p = latLonToScreen(points.get(i));
            ...
            lat = transLat(points.get(i).latitude, height) + 80;
            path.lineTo(lon, lat);
        }
