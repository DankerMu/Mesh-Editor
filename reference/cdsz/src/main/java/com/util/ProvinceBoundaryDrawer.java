 ...
 * @description TODO
 */
public class ProvinceBoundaryDrawer {
...
//		String filePathPng = "E:/fl/datas/images/deep/tp_999_deeplearning_2025112108_048_048.png";
		...
		ProvinceBoundaryDrawer drawer = new ProvinceBoundaryDrawer();
...
//        String tuli = basePath + "tuli.png";
        try {
            ...
            Font font4 = font.deriveFont(Font.BOLD, 12);
            Font font6 = font.deriveFont(Font.BOLD, 15);
            ...
            for (String city : cityMap.keySet()) {
              ...
              if ("加勒万".equals(city)) {
            	  graphics2.fillOval((int)x + 5, (int)y - 5, redis * 2, redis * 2);
              } else {
            	  graphics2.fillOval((int)x - redis, (int)y - redis, redis * 2, redis * 2);
              } 
              float lon = (float)x - (city.length() * 6);
              float lat = (float)y + 20.0F;
              if ("酒泉,武威".contains(city)) {
                lon = (float)x - (city.length() * 18);
                lat = (float)y + 5.0F;
              } else if ("塔城,重庆,班公湖,典角,林芝,错那".contains(city)) {
                lon = (float)x + 10.0F;
                lat = (float)y + 5.0F;
              } else if ("加勒万".contains(city)) {
                lon = (float)x + 15.0F;
                lat = (float)y + 1.0F;
              } else if ("万州,日喀则,洞朗,聂拉木,普兰".contains(city)) {
                lon = (float)x - (city.length() * 6);
                lat = (float)y - 10.0F;
              } else if ("拉多拉".contains(city)) {
                lon = (float)x - (city.length() * 6);
                lat = (float)y - 6.0F;
              } else if ("洞朗".contains(city)) {
                lon = (float)x - (city.length() * 6);
                lat = (float)y - 100.0F;
              } 
              ...
              graphics2.drawString(city, lon, lat);
            }
            
            graphics2.setColor(Color.BLACK);
            Font font1 = font.deriveFont(0, 35.0F);
            graphics2.setFont(font1);
            graphics2.drawString(title, image2.getWidth() / 4, addHeight);
            Font font3 = font.deriveFont(0, 25.0F);
            graphics2.setFont(font3);
            graphics2.drawString(subTitle, image2.getWidth() / 3, 90);
            Font font5 = font.deriveFont(1, 25.0F);
            graphics2.setFont(font5);
            int tuLiY = 100;
            int tuLiHeight = 26;
            
            graphics2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            graphics2.drawString("图 例", image2.getWidth() - 110, tuLiY - 10);
            int i = 0;
            Font font2 = font.deriveFont(0, 18.0F);
            graphics2.setFont(font2);
            for (int[] color : rainColorList) {
              graphics2.setColor(new Color(color[0], color[1], color[2]));
              graphics2.fillRect(width + addWidth - 80, tuLiY + i * tuLiHeight, addWidth, addHeight - 27);
              graphics2.setColor(Color.BLACK);
              graphics2.drawString(rainColorNameValues[i], width + addWidth + 58 - 80, tuLiY + i * tuLiHeight + 15);
              i++;
            } 
            for (int[] color : snowColorList) {
              ...
              graphics2.fillRect(width + addWidth - 80, tuLiY + i * tuLiHeight, addWidth, addHeight - 27);
              graphics2.setColor(Color.BLACK);
              graphics2.drawString(snowColorNameValues[i - rainColorNameValues.length], width + addWidth + 58 - 80, tuLiY + i * tuLiHeight + 15);
              i++;
            } 
...
//            Font customWindFont = null;
            try {
                ...
                customFont1 = Font.createFont(Font.TRUETYPE_FONT, fontFile).deriveFont(Font.BOLD, 30);
...
//                		graphics2.setFont(customWindFont);
                	...
                	if(j == 2)
                	{
//                		graphics2.rotate(5 * Math.PI / 4, width + addWidth - 80, tuLiY + tuLiHeight * (i - j) + j * wwFontHeight);
                		Point p = new Point(width + addWidth - 50, tuLiY + tuLiHeight * (i - j) + j * wwFontHeight);
                		Font font7 = font.deriveFont(0, 35.0F);
                		graphics2.setFont(font7);
                		new Wind().plotWind(graphics2, p, 315, 11);
                	}
                	else
                	{
                		graphics2.setFont(font2);
                		graphics2.drawString(ws.weather(wwInts[j]), width + addWidth - 80, tuLiY + tuLiHeight * (i - j) + j * wwFontHeight); // 位置(x, y)
                	}
                	graphics2.setFont(font2);
                	graphics2.setColor(Color.BLACK);
                	graphics2.drawString(wwColorNameValues[j], width + addWidth + 58 - 80, tuLiY + tuLiHeight * (i - j) + j * wwFontHeight - 6);
                }
            }
